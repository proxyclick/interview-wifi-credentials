...
describe('Search visitors', () => {
  ...
  it('Should find Tyrion', async () => {
    ...
    const tyrion = await VisitorsService.getVisitors({ email: 'tyrion@lannister.com' });
  ...
  it('Should returns all the one that works on the Wall', async () => {
    ...
    const nightWatch = await VisitorsService.getVisitors({ companyName: 'The Wall' });
  ...
  it('Should get only Jon Snow', async () => {
    ...
    const jon = await VisitorsService.getVisitors({ email: 'jon@snow.com', companyName: 'The Wall' });
